...
class Constant: NSObject {
    ...
    //MARK: -------------------------- 值类型计算属性 - 沙盒路径
    open static var SANDBOX_HOME_PATH: String {
        return NSHomeDirectory()
    }
    open static var SANDBOX_DOC_PATH: String {
        if let path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
            return path
        }
        return Constant.SANDBOX_TMP_PATH
    }
    open static var SANDBOX_LIB_PATH: String {
        if let path = NSSearchPathForDirectoriesInDomains(.libraryDirectory, .userDomainMask, true).first {
            return path
        }
        return Constant.SANDBOX_TMP_PATH
    }
    open static var SANDBOX_CACHES_PATH: String {
        if let path = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first {
            return path
        }
        return Constant.SANDBOX_TMP_PATH
    }
    open static var SANDBOX_TMP_PATH: String {
