...
- (void)itemAction:(LBSegment *)segment {
    ...
    //响应代理方法
    NSInteger index = segment.tag - 1995;
    if (index >= 0 && index < _viewControllers.count) {
        if ([self.delegate respondsToSelector:@selector(segment:didMoveToIndex:viewController:)]) {
            [self.delegate segment:self didMoveToIndex:index viewController:_viewControllers[index]];
        }
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    //响应代理方法
    if (currentIndex >= 0 && currentIndex < _viewControllers.count) {
        if ([self.delegate respondsToSelector:@selector(segment:didMoveToIndex:viewController:)]) {
            [self.delegate segment:self didMoveToIndex:currentIndex viewController:_viewControllers[currentIndex]];
        }
    }
}
